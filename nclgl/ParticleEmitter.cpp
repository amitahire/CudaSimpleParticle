...
#include <io.h>
#include <stdio.h>
//#include "../CudaSimpleParticle/CudaSimpleParticle/KeyboardMouseManager.h"
...
ParticleEmitter::ParticleEmitter(void)	{
	...
	particleInitPosition = Vector3( 0,0,0);

	/*
	Each particle is a white dot, which has an alpha fade on it,
	so the edges fade to 0.0 alpha.
	*/
	texture = SOIL_load_OGL_texture(TEXTUREDIR"particle1.jpg",
...
void ParticleEmitter::Update(float msec)	{
	...
	//if(tempflip)particleInitPosition += Vector3(5,0,0);
	//if(!tempflip)particleInitPosition -= Vector3(5,0,0);
	//if(particleInitPosition.x >= 4000){
	//	tempflip = false;
	//} 
	//if(particleInitPosition.x <= 0){
	//	tempflip = true;
	//} 
	...
	*/
	...
	//KeyboardMouseManager::GetKeyboardMouseManager().sphere_size = particles.size();
...
Particle* ParticleEmitter::GetFreeParticle()	{
	...
	//p->colour		= Vector4(1,0.5,0,1);
	//p->colour		= Vector4(1,1,1,1);
	//p->direction	= initialDirection;
	p->direction.x = ((RAND()-RAND()) * particleVariance)/50;
	p->direction.y = ((RAND()-RAND()) * particleVariance);	
	p->direction.z = ((RAND()-RAND()) * particleVariance)/50;
	
	p->direction.y = abs(p->direction.y);
	//p->direction.Normalise();	//Keep its direction normalised!
	p->position.ToZero();

	particleInitPosition = particleInitPosition;
	//cout << initPos.x << ":" << initPos.y << ":" << initPos.z <<endl;

	p->position		= particleInitPosition  + (Vector3((RAND()-RAND())*3000,0,((RAND()-RAND())*3000)));
