...
void Camera::UpdateCamera (float msec) {
	...
	if( Window :: GetKeyboard ()->KeyDown( KEYBOARD_SPACE )) {
		...
		//For CudaParticles - cheap fix.
	if( Window :: GetKeyboard () -> KeyDown ( KEYBOARD_UP )) {
		cudaParticlePos +=  Matrix4 :: Rotation ( Camera::GetYaw() , Vector3 (0.0f ,1.0f ,0.0f )) * Vector3 (0.0f ,0.0f , -1.0f) * msec ;
	}
	if( Window :: GetKeyboard () -> KeyDown ( KEYBOARD_DOWN )) {
		cudaParticlePos -= Matrix4 :: Rotation ( Camera::GetYaw() , Vector3 (0.0f ,1.0f ,0.0f )) * Vector3 (0.0f ,0.0f , -1.0f) * msec ;
	}
	if( Window :: GetKeyboard () -> KeyDown ( KEYBOARD_LEFT )) {
		cudaParticlePos += Matrix4 :: Rotation ( Camera::GetYaw() , Vector3 (0.0f ,1.0f ,0.0f )) * Vector3 (-1.0f ,0.0f ,0.0f) * msec ;
	}
	if( Window :: GetKeyboard () -> KeyDown ( KEYBOARD_RIGHT )) {
		cudaParticlePos -= Matrix4 :: Rotation ( Camera::GetYaw() , Vector3 (0.0f ,1.0f ,0.0f )) * Vector3 ( -1.0f ,0.0f ,0.0f ) * msec ;
	}
	if( Window :: GetKeyboard () -> KeyDown ( KEYBOARD_PLUS )) {
		cudaParticlePos.y -= msec ;
	}
