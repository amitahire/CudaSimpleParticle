...
*/
ParticleSystem::~ParticleSystem(void)	{
	for(thrust::host_vector<Particle *>::iterator i = particles.begin(); i != particles.end(); ++i) {
		delete (*i);
	}
	for(thrust::host_vector<Particle *>::iterator i = freeList.begin(); i != freeList.end(); ++i) {
		delete (*i);
	}
}
...
void ParticleSystem::Update(float msec)	{
	...
	*/
	...
	for(thrust::host_vector<Particle *>::iterator i = particles.begin(); i != particles.end();/*No I++ here!!! */) {
		Particle *p = (*i);
